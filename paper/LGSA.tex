...
We train one policy for each of the nine operator--strategy combinations under the common protocol; the winning pairing is carried unchanged through all subsequent experiments.
% TODO: 3x3 results table (mean final cost / eval time per cell); analysis of
% which pairings work and why (failure modes of the losing cells); statement
% of the adopted configuration.
%   sweep: V1_heur_feas (HEURISTIC x UPDATE_METHOD, 3 seeds)
%   eval:  uv run eval/run.py --dataset random --FOLDER V1_heur_feas --dim 100
%          --DATA nazari --OUTER_STEPS 10000 --dtype float16 --no-baseline
...
% TODO: LOO + thematic table; isolation table; best-set confirmation table
% (candidate sets vs all-on, 5 seeds); conditional-descriptor table
% (COND_RANK / COND_DETOUR / 2x2 combo).
% TODO: computational analysis: share of inference time spent on feature
% extraction; cost/benefit per group.
%   sweeps: V2_feat_all, V2_loo_*, V2_thematic_*, V2_iso_*, V2_best_*,
%           V2_cond_rank, V2_cond_detour, V2_cond_combo
...
Finally, two flags tame the proposal distribution without adding capacity: \textit{logit clipping}, which bounds the logits to $C \tanh(z)$ before the softmax so the distribution cannot collapse to a near-deterministic choice \citep{kool_attention_2019}, and a \textit{learnable softmax temperature} per stage. These are judged on policy entropy and update stability in addition to final cost.
% TODO: architecture tables (critic variants; EMBEDDING_DIM x NUM_H_LAYERS;
% global context / shared encoder / bilinear; logit shaping) + retained
% configuration and parameter count of the final policy.
%   sweeps: V3_critic, V3_arch, V3b_ctx, V3c_seq_ref + V3c_shared, V3d_stab
